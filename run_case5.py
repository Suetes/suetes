...
def run_case5():
    ...
    print(f"Starting Case 5 (t={t_end}s)...")
    final = sim.run(state, 0.0, t_end, dt, chunk_steps=500)

    # ======================================================
    # 6. Analytic Comparison (Full Non-Hydrostatic)
    # ======================================================
    print("Computing Exact Non-Hydrostatic Analytic Solution...")
    X = np.array(grid.X_m)
    Z = np.array(grid.Z_m)
    ...
    # Finer spectral grid for accuracy
    dk = 2.0 * np.pi / 100000.0
    k_max = 2.0 * np.pi / 250.0 # Capture fine details
    ...
    # --- FIX: Full Dispersion Relation ---
    l_sq = (N / U_FINAL)**2
    ms = csqrt(l_sq - ks**2)
    
    m_real_part = np.real(ms)
    m_imag_part = np.imag(ms)
    
    # Apply sign convention for propagating waves
    m_final = np.sign(ks) * m_real_part + 1j * m_imag_part
    ...
    for i in range(nx):
        ...
        # exp( i k x + i m z )
        phase = np.exp(1j * (ks * x_col + np.outer(z_col, m_final)))
    ...
    plt.title(f"Vertical Velocity at z={Z[0, k_slice]:.0f}m")
    plt.legend()
    plt.savefig("figures/case5_cross_section.png")
    ...
    plt.tight_layout()
    plt.savefig("figures/case5_paper_match.png")
