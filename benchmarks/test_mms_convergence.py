...
# Manufactured Solutions (Squared sines to ensure exact zero-derivative boundaries)
def u_func(x, y, z):
...
# Correct Analytical Hydrostatic Background Profiles
def th_bg_func(z):
...
# Analytical RHS Tendencies (Eulerian equations)
@jax.jit
...
def analytic_tend_pi(x, y, z):
    ...
    # Matches model's m**2 horizontal scale representation
...
def analytic_tend_th(x, y, z):
    ...
    # Matches model's m horizontal scale advection
...
def analytic_tend_w_imp(x, y, z):
    th_bg_val = th_bg_func(z)
    dpip_dz = jax.grad(lambda z_val: pip_func(x, y, z_val))(z)
    return -cp * th_bg_val * dpip_dz

# Explicit (Non-Advection) Tendencies for SISL
@jax.jit
def analytic_tend_u_exp(x, y, z):
    m = 1.0 + (x**2 + y**2) / (4.0 * R_earth**2)
    dm_dx = x / (2.0 * R_earth**2)
    dm_dy = y / (2.0 * R_earth**2)
    ...
    press_grad_nonlinear = -cp * thvp_val * m * dpip_dx
    metric = (v * dm_dx - u * dm_dy) * v  # Linear Coriolis removed
    
    return press_grad_nonlinear + metric

@jax.jit
def analytic_tend_v_exp(x, y, z):
    m = 1.0 + (x**2 + y**2) / (4.0 * R_earth**2)
    dm_dx = x / (2.0 * R_earth**2)
    dm_dy = y / (2.0 * R_earth**2)
    ...
    press_grad_nonlinear = -cp * thvp_val * m * dpip_dy
    metric = -(v * dm_dx - u * dm_dy) * u # Linear Coriolis removed
    
    return press_grad_nonlinear + metric

@jax.jit
def analytic_tend_w_exp(x, y, z):
    th_bg_val = th_bg_func(z)
    thvp_val = thvp_func(x, y, z)
    dpip_dz = jax.grad(lambda z_val: pip_func(x, y, z_val))(z)
    
    press_grad_nonlinear = -cp * thvp_val * dpip_dz
    buoyancy = g * (thvp_val / th_bg_val)
    return press_grad_nonlinear + buoyancy

@jax.jit
def analytic_tend_th_exp(x, y, z):
    dth_bg_dz = jax.grad(th_bg_func)(z)
    w = w_func(x, y, z)
    return -w * dth_bg_dz
...
vmap_u_exp = jax.vmap(jax.vmap(jax.vmap(analytic_tend_u_exp, in_axes=(0, 0, 0)), in_axes=(0, 0, 0)), in_axes=(0, 0, 0))
vmap_v_exp = jax.vmap(jax.vmap(jax.vmap(analytic_tend_v_exp, in_axes=(0, 0, 0)), in_axes=(0, 0, 0)), in_axes=(0, 0, 0))
vmap_w_exp = jax.vmap(jax.vmap(jax.vmap(analytic_tend_w_exp, in_axes=(0, 0, 0)), in_axes=(0, 0, 0)), in_axes=(0, 0, 0))
vmap_th_exp = jax.vmap(jax.vmap(jax.vmap(analytic_tend_th_exp, in_axes=(0, 0, 0)), in_axes=(0, 0, 0)), in_axes=(0, 0, 0))
...
def run_mms_at_resolution(core_type, n, num_steps=5):
    ...
    # Scale timestep linearly with grid resolution
    # Base dt = 160.0 for N = 16 (dx = 125km)
    dt = 160.0 * (16.0 / n)
    ...
    if core_type == "sisl":
        core_kwargs = {
            ...
            "damp_height": 20000.0,
            "solver_tol": 1e-12, "solver_maxiter": 100, "solver_restart": 100
        }
    ...
    # Setup exact discrete hydrostatic background profiles matching the model
    ...
    # Setup initial state corresponding to MMS using discrete backgrounds
    initial_state = {
    ...
    # --- Safe Evaluation Masks ---
    # Safe interior evaluation buffer (larger than sponge_width + acoustic travel distance)
    buffer = 450000.0 
    ...
    # --- Davies Sponge Setup ---
    sponge_width = 300000.0  # 300 km fixed physical buffer
    ...
    # Precompute analytical forcing fields at the specific grid points
    ...
    # Calculate explicit forcing based on the off-centering parameter alpha
    ...
    def get_tendencies_with_forcing(state_prime, bg, is_explicit=False, ml_params=None):
        ...
        if is_explicit:
            if core_type == "sisl":
                # For SISL explicit step, inject non-advection explicit forcing
                tends['u'] -= ana_u_exp
                tends['v'] -= ana_v_exp
                tends['w'] -= ana_w_exp
                tends['pi'] -= ana_pi
                tends['th_v'] -= ana_th_exp
                # Sponge tendencies to enforce correct steady-state analytical tendencies at boundaries
                ...
                # For split-explicit Eulerian step, inject total analytical forcing
                tends['u'] -= ana_u
                tends['v'] -= ana_v
                tends['w'] -= ana_w
                tends['pi'] -= ana_pi
    ...
    # Add implicit centering correction to GMRES RHS for SISL
    if core_type == "sisl":
    ...
    # Standardize integration time across all resolutions
    t_end_fixed = 320.0 
    num_steps = int(t_end_fixed / dt)
    actual_t_end = num_steps * dt
    
    def bc_fn(s, f):
        # Enforce boundary conditions on staggered faces corresponding to their physical boundaries
        for k in ['u', 'v', 'w']:
    ...
    def step_fn_mms(s, i):
        ...
        # Relax boundary zones towards analytical solution using the sponge mask
        ...
        # Sponge history variables if they are present in state to prevent boundary leaks
        if 'u_prev' in s_next:
...
if __name__ == "__main__":
    # Run the convergence study for both cores
