...
def run_single_worker(core_mode, T_val, grid_size, worker_type="total"):
    ...
    if core_type == "sisl":
        ...
        core_kwargs = {
            ...
            "N_bv": 0.0,
            "solver_tol": 1e-4,
            "solver_maxiter": iters,
            "solver_restart": iters,
            "alpha": 0.55,
            ...
            json.dumps(
                {
                    ...
                    "peak_increment_fwd_mb": (
                        max(0.0, peak_mem - base_mem) / (1024.0 * 1024.0) if peak_mem else float("nan")
                    ),
                }
            )
        )
    ...
    final_stats = dev.memory_stats() if hasattr(dev, "memory_stats") else None
    peak_mem = (final_stats or {}).get("peak_bytes_in_use", 0)
    mem_used_mb = max(0.0, (peak_mem - base_mem) / (1024.0 * 1024.0)) if peak_mem else float("nan")

    finite_value = bool(jnp.isfinite(val))
    finite_gradients = all(bool(jnp.all(jnp.isfinite(leaf))) for leaf in jax.tree_util.tree_leaves(grads))
...
if __name__ == "__main__":
    ...
    for mode in modes:
        ...
        for i, g in enumerate(grids):
            ...
            try:
                ...
                vram_overhead = (
                    max(0.0, vram_total - vram_fwd) if np.isfinite(vram_total) and np.isfinite(vram_fwd) else np.nan
                )
                ...
                print(
                    f"  Grid: {g[0]}x{g[1]}x{g[2]} | [WORKER FAILED: possible OOM or numerical/runtime error]",
                    flush=True,
                )
    ...
    fig, (ax_time, ax_ratio, ax_mem_total, ax_mem_delta) = plt.subplots(1, 4, figsize=(24, 5.5))
    ...
    for mode in modes:
        ...
        ax_time.plot(
            grid_cells,
            results_dom[mode]["runtimes_total"],
            "o-",
            color=c,
            label=f"{labels[mode]} (Total)",
            linewidth=2.5,
            markersize=7,
        )
        ...
        ax_time.plot(
            grid_cells,
            results_dom[mode]["runtimes_bwd"],
            "s--",
            color=c,
            label=f"{labels[mode]} (Adjoint)",
            linewidth=2.0,
            markersize=6,
            alpha=0.85,
        )
    ...
    for mode in modes:
        c = colors[mode]
        ax_ratio.plot(
            grid_cells, results_dom[mode]["ratios"], "o-", color=c, label=labels[mode], linewidth=2.5, markersize=7
        )
    ...
    for mode in modes:
        ...
        ax_mem_total.plot(
            grid_cells,
            results_dom[mode]["forward_vrams"],
            "s--",
            color=c,
            label=f"{labels[mode]} (forward)",
            linewidth=2.0,
    ...
    for mode in modes:
        c = colors[mode]
        ax_mem_delta.plot(
            grid_cells, results_dom[mode]["vrams"], "o-", color=c, label=labels[mode], linewidth=2.5, markersize=7
        )
    ...
    print(f"\n[SUCCESS] Benchmark complete. Plot saved to {figure_path}\nNumerical results saved to {csv_path}\n")
