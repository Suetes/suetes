...
if __name__ == "__main__":
    ...
    data_dir, figure_dir = setup_experiment_directories(args, kind="benchmarks", case="schaer_mountain_2d")
    ...
    artifact = xr.Dataset(
        data_vars={
            "vertical_velocity": (("x", "z_interface"), np.asarray(final_state["w"])),
            "physical_height": (("x", "z_interface"), np.asarray(grid.Z_w)),
            "terrain_height": ("x", np.asarray(hx_m)),
        },
        coords={"x": np.asarray(grid.X_w[:, 0]), "z_interface": np.arange(grid.Z_w.shape[1])},
        attrs={"dt_s": dt, "t_end_s": t_end, "core": "sisl"},
    )
    artifact_path = save_plot_dataset(artifact, data_dir / "artifact.nc", experiment="schaer_mountain_2d")
