...
class StretchedSleveSimple(BaseTransform):
    ...
    def __call__(self, xi, zeta, h, Lz):
        ...
        zeta_stretched = jnp.where(
            jnp.abs(self.kappa) > 1e-5, Lz * (jnp.exp(self.kappa * eta) - 1.0) / (jnp.exp(self.kappa) - 1.0), zeta
        )
...
def run_stress_test(
    experiment="density_current", use_stretched=True, *, data_dir: Path, figure_dir: Path, render: bool
):
    ...
    grid = RegionalGrid3D(
        nx, ny, nz, dx, dy, dz, lat_center=45.0, lon_center=0.0, h_func=h_func, transform=transform_op
    )
    ...
    artifact = xr.Dataset(
        ...
        coords={"x": np.asarray(grid.x_m), "z": np.arange(nz_plot)},
        attrs={
    ...
    artifact_path = save_plot_dataset(artifact, data_dir / f"{name}.nc", experiment="stretched_sleve_stress_test")
