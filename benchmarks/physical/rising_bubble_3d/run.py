...
def run_core(core: str, dx: float, dt: float, t_end: float, stabilization: float, snapshot_interval: float, output: Path,) -> None:
    ...
    snapshot_times = [0.0]
    theta_snapshots = [
        np.asarray(state["th_v"][:, y_mid, :] - theta_bg[:, y_mid, :])
    ]
    final_state = state
    for segment in range(num_segments):
        ...
        for chunk in range(chunks_per_snapshot):
            start_step = (
                segment * snapshot_steps + chunk * chunk_steps
            )
        ...
        theta_snapshots.append(
            np.asarray(
                final_state["th_v"][:, y_mid, :] - theta_bg[:, y_mid, :]
            )
        )
        print(
            f"    Snapshot: {current_time:6.1f}s / {t_end:.1f}s "
...
def plot_comparison(sisl_path: Path, split_path: Path, output_dir: Path, comparison_time: float) -> Path:
    ...
    def field_at_time(result: dict[str, np.ndarray]) -> np.ndarray:
        ...
        matches = np.flatnonzero(
            np.isclose(result["snapshot_times"], comparison_time)
        )
        if len(matches) != 1:
            raise ValueError(
                f"No stored snapshot at t={comparison_time:g} s; available "
                f"times are {result['snapshot_times']}"
            )
    ...
    output_path = output_dir / (
        f"rising_bubble_dual_core_3d_dx{dx:g}_dt{dt:g}"
        f"_t{comparison_time:g}"
...
def plot_evolution(sisl_path: Path, split_path: Path, output_dir: Path) -> Path:
    ...
    def moments(field):
        ...
        spread = float(
            np.sqrt(
                np.sum(positive * (z[None, :] - centroid) ** 2) / total
            )
        )
        return total, centroid, spread, float(np.max(field))

    for theta_sisl, theta_split in zip(
        sisl["theta_prime_snapshots"], split["theta_prime_snapshots"]
    ):
        difference = theta_sisl - theta_split
        mean_norm = np.linalg.norm(0.5 * (theta_sisl + theta_split))
        relative_l2.append(
            np.linalg.norm(difference) / max(mean_norm, np.finfo(float).tiny)
        )
    ...
    axes[1].plot(
        times, centroid_difference, "o-", color="#009E73",
        label="centroid difference",
    )
    axes[1].plot(
        times, spread_difference, "s--", color="#CC79A7",
    ...
    axes[2].plot(
        times, integral_ratio, "o-", color="#56B4E9",
        label="positive-anomaly integral",
    )
    axes[2].plot(
        times, peak_ratio, "s--", color="#E69F00",
        label="peak amplitude",
    )
    axes[2].plot(
        times, cosine, "^-.", color="#000000",
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / (
        f"rising_bubble_cross_core_evolution_dx{dx:g}_dt{dt:g}"
        f"{stabilization_suffix}.png"
    )
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    for values in zip(
        times, relative_l2, cosine, centroid_difference,
        integral_ratio, peak_ratio,
    ):
        print(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--comparison-time", type=float,
        help="Snapshot time for the three-panel comparison (default: t_end)",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.worker_core:
        ...
        run_core(
            ...
            args.stabilization, args.snapshot_interval, args.worker_output,
        )
        return
