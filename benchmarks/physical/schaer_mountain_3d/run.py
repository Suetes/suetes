...
def run_core(
    ...
    def mountain_jax(x, y):
        del y
        return 250.0 * jnp.exp(-((x / 5000.0) ** 2)) * jnp.cos(jnp.pi * x / 4000.0) ** 2

    grid = RegionalGrid3D(nx, ny, nz, dx, dy, dz, lat_center=45.0, lon_center=0.0, h_func=mountain_jax)
    operators = CGridOperator3D(grid)
    background_physics = Euler3D(grid, operators, CONSTANTS, dt=dt, N_bv=N_BV)
    ...
    mask_x = jnp.where(
        boundary_distance < relaxation_cells, jnp.cos(0.5 * jnp.pi * boundary_distance / relaxation_cells) ** 2, 0.0
    )[:, None, None]
    ...
    common = {
        ...
        "damp_height": 12_000.0,
        "max_damp": 0.5,
        "N_bv": N_BV,
    }
    if core == "sisl":
        kwargs = {**common, "solver_tol": 1.0e-8, "solver_maxiter": 30, "solver_restart": 30}
    ...
    stepper, actual_dt = build_dynamical_core(
        core_type=core, grid=grid, operators=operators, constants=CONSTANTS, initial_state=state, **kwargs
    )
    ...
    print(f"\n[BENCHMARK] {core.upper()} Schär: {nx}x{ny}x{nz}, dt={actual_dt:g} s, steps={num_steps}")
    print(
    ...
    for snapshot in range(num_snapshots):
        ...
        u_center = 0.5 * (current_state["u"][1:, y_mid, :] + current_state["u"][:-1, y_mid, :])
        w_center = 0.5 * (current_state["w"][:, y_mid, 1:] + current_state["w"][:, y_mid, :-1])
        momentum_flux = jnp.sum(current_state["rho"][:, y_mid, :] * (u_center - U_BACKGROUND) * w_center * dx, axis=0)
        ...
        print(f"    Snapshot: {time_now:7.1f}s / {t_end:.1f}s | Max W: {max_w:.4f} m/s")
...
def snapshot_index(result, comparison_time):
    ...
    if len(matches) != 1:
        raise ValueError(f"No snapshot at {comparison_time:g} s; available: {result['snapshot_times']}")
...
def plot_comparison(sisl_path, split_path, output_dir, comparison_time):
    ...
    x_grid = np.broadcast_to((sisl["x"][window] / 1000.0)[:, None], w_sisl.shape)
    z_grid = sisl["z_w"][window] / 1000.0

    fig, axes = plt.subplots(1, 3, figsize=(18, 5.4), sharex=True, sharey=True, constrained_layout=True)
    images = []
    for axis, field, title in zip(axes[:2], (w_sisl, w_split), ("(a) SISL", "(b) Split-Explicit")):
        image = axis.contourf(
            x_grid, z_grid, field, levels=np.linspace(-main_limit, main_limit, 61), cmap="RdBu_r", extend="both"
        )
        images.append(image)
    ...
    main_colorbar = fig.colorbar(images[0], ax=axes[:2], shrink=0.91, label=r"Vertical velocity $w$ (m s$^{-1}$)")
    difference_colorbar = fig.colorbar(
        difference_image, ax=axes[2], shrink=0.91, label=r"Difference in $w$ (m s$^{-1}$)"
    )
    ...
    flux_l2, flux_cosine, flux_max = agreement(
        sisl["momentum_flux_snapshots"][index], split["momentum_flux_snapshots"][index]
    )
...
def plot_evolution(sisl_path, split_path, output_dir):
    ...
    for w_sisl, w_split, f_sisl, f_split in zip(
        sisl["w_snapshots"], split["w_snapshots"], sisl["momentum_flux_snapshots"], split["momentum_flux_snapshots"]
    ):
        metrics = agreement(w_sisl[window], w_split[window])
        wave_l2.append(metrics[0])
        wave_cosine.append(metrics[1])
        wave_max.append(metrics[2])
        metrics = agreement(f_sisl, f_split)
        flux_l2.append(metrics[0])
        flux_cosine.append(metrics[1])
...
def main():
    ...
    paths = {
        core: result_path(
            data_dir, core, args.dx, args.dz, args.dt, args.t_end, args.stabilization, args.snapshot_interval
        )
    ...
    dataset = xr.Dataset(
        data_vars={
            ...
            "momentum_flux": (
                ("core", "time", "z_level"),
                np.stack([sisl["momentum_flux_snapshots"], split["momentum_flux_snapshots"]]),
            ),
            "physical_height_interface": (("x", "z_interface"), sisl["z_w"]),
    ...
    artifact = save_plot_dataset(dataset, data_dir / "artifact.nc", experiment="schaer_mountain_dual_core_3d")
