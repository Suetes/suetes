...
suite.add_tendency_scheme(McFarlaneGWD(grid, op, constants, h_variance=synthetic_h_var))
...
def main():
    ...
    data_dir, figure_dir = setup_experiment_directories(args, kind="benchmarks", case="schaer_mountain_physics_2d")
    ...
    artifact = xr.Dataset(
        data_vars={
            ...
            "physical_height": (("x", "z_interface"), np.asarray(grid.Z_w[:, 1, :])),
            "terrain_height": ("x", np.asarray(schaer_mountain(grid.x_m, 0.0))),
        },
        coords={"x": np.asarray(grid.x_m), "z_interface": np.arange(nz + 1)},
        attrs={"dt_s": dt, "t_end_s": t_end, "core": "sisl"},
    )
    artifact_path = save_plot_dataset(artifact, data_dir / "artifact.nc", experiment="schaer_mountain_physics_2d")
