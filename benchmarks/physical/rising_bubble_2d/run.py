...
def get_initial_state(grid, physics):
    ...
    rho_bg = (physics.c["p0"] / (physics.c["Rd"] * theta_bg)) * (pi_bg ** (physics.c["cvd"] / physics.c["Rd"]))
...
if __name__ == "__main__":
    ...
    artifact = xr.Dataset(
        ...
        coords={"x": np.asarray(grid.X_m[:, 0]), "z": np.asarray(grid.Z_m[0, :])},
        attrs={"dt_s": dt, "t_end_s": t_end, "core": "sisl"},
    )
    artifact_path = save_plot_dataset(artifact, data_dir / "artifact.nc", experiment="rising_bubble_2d")
    ...
    plt.title(rf"SISL Rising Bubble: $\Delta \theta$ at T={t_end}s (dt={dt}s)")
    plt.xlabel("x (km)")
    plt.ylabel("z (km)")
