...
def vmap_bicubic_interp(field, coords, periodic_x=False, true_nx=None):
    """
    Natively vectorized 16-point bicubic interpolation.
    """
    nx_arr, nz = field.shape
    # Use true_nx for periodic math if provided, otherwise fallback to array shape
    ...
    if periodic_x:
        x = x % nx  # Wrap using the true physical domain size
...
class SemiLagrangianAdvector:
    ...
    def _get_logical_velocities(self, u_phys, eta_dot, loc='m'):
        """Uses the Semi-Lagrangian eta_dot directly, completely bypassing Eulerian metric distortion!"""
        if loc == 'm':
            u_loc = self.physics.op.avg_u_to_m(u_phys)
            w_loc = self.physics.op.avg_w_to_m(eta_dot)
        elif loc == 'u':
            u_loc = u_phys
            w_loc = self.physics.op.avg_w_to_u(eta_dot)
        elif loc == 'w':
    ...
    def compute_departure_indices(self, u_phys, eta_dot, loc='m', iterations=1):
        ...
        for _ in range(iterations):
            ...
            u_mid = vmap_bicubic_interp(u_log, coords, periodic_x=self.grid.periodic_x)
            w_mid = vmap_bicubic_interp(w_log, coords, periodic_x=False) 
    ...
    def advect(self, field, u_phys, w_phys, loc='m'):
        ...
        return vmap_bicubic_interp(field, coords, periodic_x=self.grid.periodic_x)
...
class SemiImplicitSolver:
    ...
    def precompute_bg(self, bg_state):
        ...
        # Add the face-centered vertical thickness metric
    ...
    def get_tendencies(self, state_prime, bg):
        """Computes the spatial gradients and divergence for the acoustics."""
        ...
        # NEW: Perfect alignment with Semi-Lagrangian eta_dot (Equation 21)
    ...
    def linear_operator(self, state_prime, bg):
        ...
        L_u = state_prime['u'] - 0.5 * self.dt * tends['u']
        # Fixed: Included the tau_damp layer into the implicit LHS!
        L_w = (1.0 + self.dt * self.physics.tau_damp) * state_prime['w'] - 0.5 * self.dt * tends['w']
        L_pi = state_prime['pi'] - 0.5 * self.dt * tends['pi']

        # NEW: Equation 37 LHS Constraint mapped natively into GMRES
...
class SISLStepper:
    ...
    def step(self, state, t, forcing, bc_fn):
        ...
        # 2. --- CRANK-NICOLSON EXPLICIT HALF (Time N) ---
        # BUG FIX: Compute the exact stationary reference density
        rho_bg = (self.physics.c['p0'] / (self.physics.c['Rd'] * self.physics.theta_bg)) * \
                 (self.physics.pi_bg ** (self.physics.c['cvd'] / self.physics.c['Rd']))

        # The linear kernel MUST use the stationary reference state, NOT the perturbed state!
        bg_state_ref = {
            'rho': rho_bg,
        ...
        # MISSING LINES RESTORED: Form intermediate variables: x^n + 0.5 * dt * F(x^n)
        u_adv_in = state['u'] + 0.5 * self.dt * tends_n['u']
        w_adv_in = state['w'] + 0.5 * self.dt * tends_n['w']
        pi_prime_adv_in = state_prime_n['pi'] + 0.5 * self.dt * tends_n['pi']

        # NEW: Equation 37 RHS Geometric Evaluation for eta_dot (R_eta_dot)
        ...
        th_v_next = vmap_bicubic_interp(state['th_v'], coords_m, periodic_x, true_nx=nx)

        # 4. --- CRANK-NICOLSON IMPLICIT HALF (Time N+1) ---
        # (buoyancy_next logic remains the same)
        ...
        rhs_w += 0.5 * self.dt * buoyancy_next

        rhs_prime = {
            'u': rhs_u, 'w': rhs_w, 'pi': rhs_pi_prime, 'eta_dot': R_eta_dot
        }
