...
class ICON2DSlice:
    def __init__(self, grid, constants, damp_height=20000.0, max_damp=0.5, N_bv=0.01):
        ...
        # 1. Generate 1D flat logical coordinates for the reference state
        zeta_c = jnp.linspace(0, self.grid.Lz, self.grid.nz + 1)
        zeta_m = 0.5 * (zeta_c[:-1] + zeta_c[1:])
        
        # Broadcast to 2D arrays so they match the expected shapes, but contain NO horizontal gradients
        Z_ref_m = jnp.broadcast_to(zeta_m, self.grid.Z_m.shape)
        Z_ref_w = jnp.broadcast_to(zeta_c, self.grid.Z_w.shape)
        
        # 2. Compute background states purely as a function of logical height
        if N_bv > 0.0:
        ...
        # Compute discrete metrics here, after grid geometry is finalized
        self.metrics = {
    ...
    def _compute_discrete_metrics(self, Z_grid):
        ...
        # Access periodic_x safely, defaulting to False
    ...
    def _get_metrics(self, grid_type: str):
        """Fetch the exact discrete metrics computed during initialization."""
