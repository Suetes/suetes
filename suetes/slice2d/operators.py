...
class CGridOperator:
    ...
    def tvd_interp_m_to_u(self, f, u):
        ...
        nx = f.shape[0]
        f_LL, f_L, f_R, f_RR = f_pad[0:nx+1], f_pad[1:nx+2], f_pad[2:nx+3], f_pad[3:nx+4]

        dq_upwind_L, dq_local_L = f_L - f_LL, f_R - f_L
        dq_upwind_R, dq_local_R = f_RR - f_R, f_R - f_L
        
        def van_leer(a, b):
            return (a * jnp.abs(b) + jnp.abs(a) * b) / (jnp.abs(a) + jnp.abs(b) + 1e-15)

        lim_L, lim_R = van_leer(dq_upwind_L, dq_local_L), van_leer(dq_upwind_R, dq_local_R)
    ...
    def tvd_interp_m_to_w(self, f, w):
        ...
        f_LL, f_L, f_R, f_RR = f_pad[:, 0:nz+1], f_pad[:, 1:nz+2], f_pad[:, 2:nz+3], f_pad[:, 3:nz+4]
        ...
        lim_L, lim_R = van_leer(dq_upwind_L, dq_local_L), van_leer(dq_upwind_R, dq_local_R)
        return jnp.where(w > 0, f_L + 0.5 * lim_L, f_R - 0.5 * lim_R)
    ...
    def grad_2d_curvilinear(self, f, metrics):
        ...
        if metrics is None: return d_xi, d_zeta
        grad_z = d_zeta / metrics['z_zeta']
        grad_x = d_xi - (metrics['z_xi'] * grad_z)
    ...
    def advect_2d(self, f, u, w, loc, metrics=None):
        if loc == 'mass':
            ...
            u_loc, w_loc = u, self.avg_m_to_u(self.avg_w_to_m(w)) 
        elif loc == 'w':
            u_loc, w_loc = self.avg_m_to_w(self.avg_u_to_m(u)), w
