...
def test_cfl_stability_limit(grid, constants, state):
    ...
    try:
        stepper, actual_dt = build_dynamical_core("split-explicit", grid, CGridOperator3D(grid), constants, state, dt=6.0, ns=20, alpha=0.55)
    ...
    stepper_sisl, actual_dt_sisl = build_dynamical_core(
        "sisl", grid, CGridOperator3D(grid), constants, state, dt=10.0, nu_div_factor=0.0, nu_h_factor=0.0,
        damp_height=7500.0, max_damp=0.05, N_bv=0.0, solver_tol=1e-6, solver_maxiter=20, solver_restart=20, alpha=0.55
    )
    sim_sisl = Simulation(step_fn=lambda s, i: (stepper_sisl.step(s, i*actual_dt_sisl, None, lambda x, f: x), 0.0), dt=actual_dt_sisl)
    s_sisl = sim_sisl.run(state, t_start=0.0, t_end=30.0, chunk_steps=int(30.0/actual_dt_sisl))
    print(f"  -> [CONFIRMED] SISL IS UNCONDITIONALLY STABLE at dt=10.0s! Max w = {float(jnp.max(s_sisl['w'])):.2f} m/s\n")
...
def run_simulation(core_type, dt, state, grid, constants, t_end=120.0):
    ...
    if core_type == "sisl":
        core_kwargs = {
            ...
            "solver_tol": 1e-6, "solver_maxiter": 20, "solver_restart": 20, "alpha": 0.55
        }
    else:
    ...
    for c in range(num_chunks):
        ...
        print(f"  T={t_end_chunk:3.0f}s | Enstrophy={enst:.2e} | Thermal Var={th_var:.2e} | Max W={max_w:.2f} m/s", flush=True)
...
if __name__ == "__main__":
    ...
    if args.output_dir is None:
        layout = ArtifactLayout(
            kind="experiments", case="core_separation_showcase",
    ...
    state_se, t_se, enst_se, var_se = run_simulation("split-explicit", dt=1.0, state=initial_state, grid=grid, constants=constants, t_end=t_end)
    jax.clear_caches()
    
    # 2. SISL Fair Comparison at Equal Timestep dt = 1.0s (Demonstrates inherent interpolation dissipation)
    state_sisl_fair, t_sisl_fair, enst_sisl_fair, var_sisl_fair = run_simulation("sisl", dt=1.0, state=initial_state, grid=grid, constants=constants, t_end=t_end)
    jax.clear_caches()
    
    # 3. SISL Operational Comparison at Large Timestep dt = 10.0s (Demonstrates operational efficiency & filtering)
    state_sisl_op, t_sisl_op, enst_sisl_op, var_sisl_op = run_simulation("sisl", dt=10.0, state=initial_state, grid=grid, constants=constants, t_end=t_end)
    ...
    print(f"SISL Equal-Timestep (1.0s) Enstrophy: {enst_sisl_fair[-1]:.2e} ({enst_diff_fair:.1f}% inherent numerical dissipation!)")
    print(f"SISL Large-Timestep (10s) Enstrophy:  {enst_sisl_op[-1]:.2e} ({enst_diff_op:.1f}% operational subgrid filtering!)")
    ...
    dataset = xr.Dataset(
        data_vars={
            "vorticity": (
                ("configuration", "x", "z"),
                np.stack([vort_se, vort_sisl_fair, vort_sisl_op]),
            ),
            "enstrophy": (
                ("configuration", "time"),
                np.asarray([enst_se, enst_sisl_fair, enst_sisl_op]),
            ),
            "thermal_variance": (
                ("configuration", "time"),
                np.asarray([var_se, var_sisl_fair, var_sisl_op]),
            ),
            "kinetic_energy_spectrum": (
                ("configuration", "wavenumber"),
                np.asarray([power_se, power_fair, power_op]),
            ),
        },
        coords={
            "configuration": [
                "split-explicit", "sisl-equal-timestep", "sisl-operational"
            ],
    ...
    artifact = save_plot_dataset(
        dataset, data_dir / "artifact.nc",
        experiment="core_separation_showcase",
    )
    ...
    ax1.set_title(r'Split-Explicit Core ($\Delta t = 1.0$s): Convective Rotors at $x=0$', fontsize=14, fontweight='bold')
    ...
    ax2.set_title(r'SISL Equal-Timestep ($\Delta t = 1.0$s): Inherent Interpolation Damping at $x=0$', fontsize=14, fontweight='bold')
    ...
    ax3.set_title(r'SISL Operational ($\Delta t = 10.0$s): Subgrid Filtered Convection at $x=0$', fontsize=14, fontweight='bold')
    ...
    ax4.plot(t_se, enst_se, 'o-', color='#1f77b4', label=r'Split-Explicit ($\Delta t = 1.0$s)', linewidth=2.5, markersize=7)
    ax4.plot(t_sisl_fair, enst_sisl_fair, '^--', color='#2ca02c', label=fr'SISL Equal-Timestep ($\Delta t = 1.0$s, -{enst_diff_fair:.1f}%)', linewidth=2.5, markersize=7)
    ax4.plot(t_sisl_op, enst_sisl_op, 's-', color='#ff7f0e', label=fr'SISL Operational ($\Delta t = 10.0$s, -{enst_diff_op:.1f}%)', linewidth=2.5, markersize=7)
    ...
    ax5.loglog(k_se, power_se, 'o-', color='#1f77b4', label=r'Split-Explicit ($\Delta t = 1.0$s)', linewidth=2.5, markersize=5)
    ax5.loglog(k_fair, power_fair, '^--', color='#2ca02c', label=r'SISL Equal-Timestep ($\Delta t = 1.0$s)', linewidth=2.5, markersize=5)
    ax5.loglog(k_op, power_op, 's-', color='#ff7f0e', label=r'SISL Operational ($\Delta t = 10.0$s)', linewidth=2.5, markersize=5)
