...
if __name__ == "__main__":
    ...
    if args.output_dir is None:
        layout = ArtifactLayout(
            kind="experiments", case="core_quantitative_comparison",
    ...
    dataset = xr.Dataset(
        data_vars={
            "kinetic_energy": (
                ("core", "time"), np.asarray([tke_se, tke_sisl])
            ),
            "enstrophy": (
                ("core", "time"), np.asarray([enst_se, enst_sisl])
            ),
            "maximum_vertical_velocity": (
                ("core", "time"), np.asarray([w_se, w_sisl])
            ),
            "vertical_velocity_spectrum": (
                ("core", "wavenumber"), np.asarray([p_se, p_sisl])
            ),
        },
        coords={
            "core": ["split-explicit", "sisl"],
    ...
    artifact = save_plot_dataset(
        ...
        experiment="core_quantitative_comparison",
    )
    ...
    axes[0].plot(t_se, enst_se, 'o-', color='#1f77b4', label=r'Split-Explicit ($\Delta t = 1.0$s)', linewidth=2, markersize=4)
