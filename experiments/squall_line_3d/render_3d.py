...
def select_volume(dataset, requested_time):
    ...
    if "cloud_water_3d" in dataset:
        ...
        return (
            float(available[index]),
            dataset.cloud_water_3d.values[index],
            dataset.rain_water_3d.values[index],
        )
    final_time = float(dataset.time.values[-1])
    if not np.isclose(requested_time, final_time):
        raise ValueError(
            "This older artifact stores a full 3-D volume only at the final time"
        )
...
def render(
    ...
    time_index = int(
        np.argmin(np.abs(np.asarray(dataset.time.values) - selected_time))
    )
    theta_xz = (
        dataset.dry_potential_temperature_xz.values[time_index]
        - dataset.environment_dry_potential_temperature.values
    )
    theta_surface = (
    ...
    norm = colors.TwoSlopeNorm(
        vmin=-theta_limit, vcenter=0.0, vmax=theta_limit
    )
    cmap = plt.get_cmap("RdBu_r")
    surface_colors = np.asarray(
        cmap(norm(np.clip(theta_surface, -theta_limit, theta_limit))),
        dtype=np.float64,
    )
    if boundary_mask_km > 0.0:
        ...
        plot_interior = (
            (boundary_x[:, None] >= boundary_mask_km)
            & (boundary_y[None, :] >= boundary_mask_km)
        )
    ...
    vertical_colors = np.asarray(
        cmap(norm(np.clip(theta_xz, -theta_limit, theta_limit))),
        dtype=np.float64,
    )
    axis.plot_surface(
        xx_vertical,
    ...
    spacing = (dx, dx, dz)
    add_isosurface(
        axis, rain, QR_THRESHOLD, origin, spacing,
    ...
    axis.set_title(
        f"Three-dimensional supercell at "
        f"$t={selected_time / 3600.0:g}$ h",
        pad=16,
    )
...
def render_rain_top_view(artifact: Path, output: Path):
    ...
    filled = axis.contourf(
        xx, yy, rain, levels=levels, cmap="Blues", extend="max"
    )
    if np.nanmax(cloud) >= QC_THRESHOLD:
        axis.contour(
            xx, yy, cloud, levels=[QC_THRESHOLD],
            colors="#777777", linewidths=0.9,
        )
    if np.nanmax(rain_water) >= QR_THRESHOLD:
        axis.contour(
            xx, yy, rain_water, levels=[QR_THRESHOLD],
            colors="#8c2d04", linewidths=1.0,
        )
    ...
    axis.set_ylim(float(y.min()), float(y.max()))
    axis.set_title(
        f"Surface rain accumulation at "
        f"$t={float(dataset.time.values[-1]) / 3600.0:g}$ h"
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output, dpi=300, bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def main():
    ...
    parser.add_argument(
        ...
        default=Path("output/squall_line_3d_dx400_nodiv/artifact.nc"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(
            "output/squall_line_3d_dx400_nodiv/squall_line_3d_isosurfaces.png"
        ),
    )
    parser.add_argument(
        "--mesh-step",
        type=int,
        default=2,
        help="marching-cubes stride; use 1 for the highest-quality mesh",
    )
    parser.add_argument(
        "--elevation",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="time in seconds of the stored 3-D volume to render",
    )
    parser.add_argument(
        "--rain-output",
        type=Path,
        default=None,
        help="optional standalone top-view accumulated-rain figure",
    )
    parser.add_argument(
        "--boundary-mask-km",
        type=float,
    ...
    if not args.artifact.is_file():
        raise FileNotFoundError(
            f"Artifact is not a file: {args.artifact!s}. "
            "If using $ARTIFACT, define it in the current shell first."
        )
    if args.mesh_step < 1:
