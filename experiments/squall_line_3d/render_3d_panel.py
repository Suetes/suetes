...
def draw_storm(
    ...
    surface_colors = np.asarray(
        cmap(norm(np.clip(theta_surface, norm.vmin, norm.vmax))),
        dtype=np.float64,
    )
    axis.plot_surface(
        ...
        antialiased=False, shade=False, alpha=0.88, zorder=0,
    )
    ...
    vertical_colors = np.asarray(
        cmap(norm(np.clip(theta_xz, norm.vmin, norm.vmax))),
        dtype=np.float64,
    )
    axis.plot_surface(
        xx_vertical, y_vertical, zz_vertical,
        facecolors=vertical_colors,
        rstride=1, cstride=1, linewidth=0,
        antialiased=False, shade=False, alpha=0.72, zorder=1,
    )
    ...
    axis.text2D(
        0.50, 0.91, f"$t={selected_time / 3600.0:g}$ h",
...
def render(artifact, output, mesh_step, elevation, azimuth, cloud_alpha):
    ...
    for axis, time_s in zip(axes_3d, (1800.0, 7200.0)):
        draw_storm(
            axis, dataset, time_s, norm, cmap, mesh_step,
            elevation, azimuth, cloud_alpha,
        )

    theta_bar_axis = fig.add_axes((0.885, 0.575, 0.014, 0.265))
    theta_bar = fig.colorbar(
        ScalarMappable(norm=norm, cmap=cmap), cax=theta_bar_axis
    )
    ...
    axes_3d[0].text2D(
        0.03, 0.88, "(a)", transform=axes_3d[0].transAxes,
        fontweight="bold",
    )
    axes_3d[1].text2D(
        0.03, 0.88, "(b)", transform=axes_3d[1].transAxes,
        fontweight="bold",
    )
    rain_axis.text(
        0.01, 0.97, "(c)", transform=rain_axis.transAxes,
...
def main():
    ...
    render(
        args.artifact, args.output, args.mesh_step,
        args.elevation, args.azimuth, args.cloud_alpha,
    )
