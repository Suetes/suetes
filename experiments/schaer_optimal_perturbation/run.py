...
if args.output_dir is None:
    layout = ArtifactLayout(
        kind="experiments", case="schaer_mountain_optimal_perturbation_3d",
...
artifact = xr.Dataset(
    data_vars={
        ...
        "vertical_velocity": (
            ...
            np.stack(validation_velocity),
        ),
        "validation_energy": (
            "configuration", np.asarray(validation_energy)
        ),
        "thermal_sensitivity": (
            ("x", "z"), np.asarray(sensitivities_cpu["th_v"][:, 1, :])
        ),
        "wind_sensitivity": (
            ("x", "z"), np.asarray(u_sens_m)
        ),
        "terrain_height": (
    ...
    attrs={
        "core": CORE_TYPE, "dt_s": dt, "t_end_s": t_end,
        "baseline_energy": float(J_0),
    },
)
artifact_path = save_plot_dataset(
    artifact, data_dir / "artifact.nc",
    experiment="schaer_mountain_optimal_perturbation_3d",
)
