...
stepper, _ = build_dynamical_core(
    core_type=CORE_TYPE, grid=grid, operators=op, constants=constants, initial_state=initial_state, **core_kwargs
)
...
def plot_field(data, title, filename, cmap="RdBu_r", add_box=False):
    ...
    contour = plt.contourf(
        X_plot_curr, Z_plot_curr, data_to_plot, levels=jnp.linspace(-vmax, vmax, 41), cmap=cmap, extend="both"
    )
...
def evaluate_perturbation(pert_th_v):
    ...
    perturbed_state["rho"] = (
        constants["p0"]
        / (constants["Rd"] * perturbed_state["th_v"])
...
for idx, (title, pert_th_v) in enumerate(configs_to_test):
    ...
    contour = ax.contourf(
        X_plot_curr, Z_w_plot, w_diff, levels=jnp.linspace(-vmax_shared, vmax_shared, 41), cmap="RdBu_r", extend="both"
    )
...
if not args.no_render:
    plt.savefig(figure_dir / "schaer_adjoint_validation.png", dpi=150, bbox_inches="tight")
print(f"\nValidation complete. Figures directory: {figure_dir}")
...
for idx, (title, pert_th_v) in enumerate(configs_to_test):
    ...
    validation_velocity.append(np.asarray(final_pert_state["w"][:, 1, :]))

    ax = axs2[idx]
    contour2 = ax.contourf(
        X_plot_curr,
        Z_w_plot,
        final_pert_state["w"][:, 1, :],
...
if not args.no_render:
    plt.savefig(figure_dir / "schaer_forward_validation.png", dpi=150, bbox_inches="tight")
print(f"Forward comparison complete. Figures directory: {figure_dir}")
...
if not args.no_render:
    plt.savefig(figure_dir / "schaer_perturbation_validation.png", dpi=150, bbox_inches="tight")
artifact = xr.Dataset(
    data_vars={
        "initial_perturbation": (
            ("configuration", "x", "z"),
            np.stack([np.asarray(value[:, 1, :]) for _, value in configs_to_test]),
        ),
        "vertical_velocity": (("configuration", "x", "z_interface"), np.stack(validation_velocity)),
        "validation_energy": ("configuration", np.asarray(validation_energy)),
        "thermal_sensitivity": (("x", "z"), np.asarray(sensitivities_cpu["th_v"][:, 1, :])),
        "wind_sensitivity": (("x", "z"), np.asarray(u_sens_m)),
        "terrain_height": ("x", np.asarray(schaer_mountain(grid.x_m, 0.0))),
    },
    coords={
    ...
    attrs={"core": CORE_TYPE, "dt_s": dt, "t_end_s": t_end, "baseline_energy": float(J_0)},
)
artifact_path = save_plot_dataset(
    artifact, data_dir / "artifact.nc", experiment="schaer_mountain_optimal_perturbation_3d"
)
