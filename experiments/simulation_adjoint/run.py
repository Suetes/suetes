...
def main():
    ...
    if args.output_dir is None:
        layout = ArtifactLayout(
            kind="experiments", case="run_simulation_adjoint",
    ...
    physics = Euler3D(grid, operators, constants, dt=dt, initial_era5_state=initial_state, damp_height=9000.0, max_damp=3.0, nu_div_factor=0.1, nu_h_factor=0.1, physics_suite=physics_suite)
    ...
    artifact = xr.Dataset(
        data_vars={
            "thermal_impact_percentage": (
                ("x", "y"), np.asarray(impact_percentage)
            ),
            "wind_sensitivity_magnitude": (
                ("x", "y"), np.asarray(wind_sens_mag)
            ),
            "vertical_velocity_sensitivity_levels": (
                ("level", "x", "y"),
                np.stack([
                    np.asarray(sensitivities_cpu["w"][:, :, index])
                    for index in level_indices
                ]),
            ),
            "thermal_sensitivity_cross_section": (
                ("x", "z"),
                np.asarray(sensitivities_cpu["th_v"][:, mid_y, :]),
            ),
            "vertical_velocity_cross_section": (
                ("x", "z_interface"),
                np.asarray(sensitivities_cpu["w"][:, mid_y, :]),
            ),
        },
        coords={
            "x": np.asarray(grid.x_m), "y": np.asarray(grid.y_m),
            "z": np.arange(grid.nz), "z_interface": np.arange(grid.nz + 1),
            "level": level_indices,
        },
        attrs={
            "domain": ACTIVE_DOMAIN, "forecast_hours": sim_hours,
            "target_metric": J_0_cpu,
        },
    )
    artifact_path = save_plot_dataset(
        artifact, data_dir / "artifact.nc",
        experiment="run_simulation_adjoint",
    )
    ...
    visualizer.plot_adjoint_overlay(
        grid, initial_state, impact_percentage,
        z_idx=5, sponge_depth=sponge_depth, stride=10,
        save_path=str(figure_dir / "adjoint_impact_percentage.png"),
    )

    visualizer.plot_2d_field(
        grid, initial_state, variable='pi', plot_data=wind_sens_mag,
        z_idx=2, sponge_depth=sponge_depth, cmap='hot', scale='linear',
    ...
    visualizer.plot_level_strip(
        grid, sensitivities, 'w', 
        z_indices=[2, 10, 18, 26], sponge_depth=sponge_depth,
    ...
    visualizer.plot_cross_section(
        grid, sensitivities, 'th_v', y_idx=mid_y, 
        sponge_depth=sponge_depth, overlay_isentropes=False, 
        save_path=str(figure_dir / "adjoint_thermal_cross_section.png")
    )
    
    visualizer.plot_cross_section(
        grid, sensitivities, 'w', y_idx=mid_y, 
        sponge_depth=sponge_depth, overlay_isentropes=False, 
