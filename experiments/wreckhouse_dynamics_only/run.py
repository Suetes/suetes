"""Dynamics-only Wreckhouse adverse-scenario experiment.
...
3. Evaluates that perturbation with the same dynamics-only configuration
   through t=9h.

This is an independent sensitivity experiment.  It intentionally omits the
Smagorinsky--Lilly SGS and McFarlane gravity-wave-drag schemes used by
``runs/wreckhouse_worst_case.py`` and writes to a separate output directory.
"""
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="output/wreckhouse_dynamics_only",
        help="Flat directory for diagnostics, plot data, metadata, and figures",
    )
    args = parser.parse_args()

    ACTIVE_DOMAIN = "wreckhouse"
    RUN_NAME = f"{ACTIVE_DOMAIN}_dynamics_only_feb2025"
    ...
    # =====================================================================
    # 2. DYNAMICS-ONLY FORWARD AND DIFFERENTIABLE CONTROL MODELS
    # =====================================================================
    ...
    # The forward and control models both omit subgrid physical
    # parameterizations.  The control model retains stronger numerical
    # stabilization to suppress grid-scale noise in the reverse trajectory.
    ...
    np.savez_compressed(
        ...
        evaluation_model="dry resolved dynamics; nu_h=nu_div=0.05",
    ...
    artifact_path = save_plot_dataset(
        ...
        experiment="wreckhouse_dynamics_only",
        metadata={
            "control_model": "dry resolved dynamics; nu_h=nu_div=0.20",
            "evaluation_model": "dry resolved dynamics; nu_h=nu_div=0.05",
            "subgrid_physics": "disabled",
        },
    )
