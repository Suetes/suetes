...
def main():
    ...
    try:
        artifact = artifact_from_bundle(args.artifact, "tracer_inversion_3d_complex_*.nc")
    except ValueError as error:
        parser.error(str(error))
    if not artifact.exists():
        parser.error(f"artifact does not exist: {artifact}. Run experiments/tracer_inversion_3d_complex/run.py first.")
    ...
    fig_plume, axes_plume = plt.subplots(2, count, figsize=(4 * count + 1.5, 7.5), sharex=True, squeeze=False)
    mappable = None
    for column in range(count):
        plume = data.plume_snapshot.isel(snapshot=column).values
        axis = axes_plume[0, column]
        mappable = axis.contourf(x, y, plume[:, :, true_z_index].T, levels=levels, cmap="Blues", extend="max")
        axis.contourf(x, y, terrain.T, levels=20, cmap="binary", alpha=0.15)
        axis.contour(x, y, terrain.T, levels=[0.2, 0.5, 0.8, 1.1, 1.4, 1.7], colors="black", alpha=0.2, linewidths=0.8)
        axis.scatter(true[0], true[1], color="green", marker="x", s=100)
        axis.scatter(sensors[:, 0], sensors[:, 1], color="red", marker="^", s=80, edgecolor="black")
        ...
        plume_padded = np.concatenate([plume[:, true_y_index, :1], plume[:, true_y_index, :]], axis=1)
        axis_z.contourf(
            np.broadcast_to(x[:, None], z_padded.shape).T,
            z_padded.T,
            plume_padded.T,
            levels=levels,
            cmap="Blues",
        ...
        axis_z.fill_between(x, 0, terrain_y, color="gray", alpha=0.5)
    ...
    fig_plume.tight_layout(rect=[0, 0, 0.88, 1.0])
    color_axis = fig_plume.add_axes([0.90, 0.15, 0.02, 0.70])
    ...
    horizontal.contourf(x, y, terrain.T, levels=20, cmap="binary", alpha=0.15)
    horizontal.contour(
        x, y, terrain.T, levels=[0.2, 0.5, 0.8, 1.1, 1.4, 1.7], colors="black", alpha=0.2, linewidths=0.8
    )
    horizontal.scatter(
        sensors[:, 0], sensors[:, 1], color="red", marker="^", s=80, edgecolor="black", label="Sensor towers"
    )
    horizontal.plot(history[:, 0], history[:, 1], "o-", color="purple", markersize=4, label="Optimizer trajectory")
    horizontal.scatter(*history[0, :2], color="orange", s=100, label="Initial guess")
    horizontal.scatter(*recovered[:2], color="purple", marker="*", s=150, label="Recovered source")
    horizontal.scatter(*true[:2], color="green", marker="x", s=120, label="True source")
    horizontal.set(xlabel="x (km)", ylabel="y (km)", xlim=(-12.5, 12.5), ylim=(-12.5, 12.5))
    horizontal.grid(True, ls="--", alpha=0.4)

    vertical.plot(history[:, 0], history[:, 2], "o-", color="purple", markersize=4, label="Optimizer trajectory")
    vertical.scatter(history[0, 0], history[0, 2], color="orange", s=100, label="Initial guess")
    vertical.scatter(recovered[0], recovered[2], color="purple", marker="*", s=150, label="Recovered source")
    vertical.scatter(true[0], true[2], color="green", marker="x", s=120, label="True source")
    vertical.fill_between(x, 0, terrain.max(axis=1), color="gray", alpha=0.3, label="Max terrain profile")
    vertical.set(xlabel="x (km)", ylabel="Altitude z (km)", xlim=(-12.5, 12.5), ylim=(0, 5))
    ...
    for sensor, axis in enumerate(axes_sens.flat):
        axis.plot(minutes, target[:, sensor], "k-", linewidth=2.5, label="True observations")
        axis.plot(minutes, inferred[:, sensor], "r--", linewidth=2.0, label="Inferred plume")
