...
ap.add_argument(
    "--core",
    type=str,
    default="split-explicit",
    choices=["sisl", "split-explicit"],
    help="Dynamical core type to run ('sisl' or 'split-explicit')",
)
ap.add_argument("--steps", type=int, default=100, help="Number of optimization steps")
ap.add_argument(
    "--output-root", default="output", help="Root containing benchmark, experiment, run, and verification bundles"
)
ap.add_argument("--name", default="default", help="Execution label")
ap.add_argument("--output-dir", help="Explicit legacy output directory")
ap.add_argument("--plot-inline", action="store_true", help="Also run the legacy in-process plotting block")
args = ap.parse_args()
...
output_dir = str(
    resolve_data_dir(
        kind="experiments",
        case="tracer_inversion_3d_complex",
        execution=args.name,
...
sensor_locs = [(-2.0, -2.0, 1.0), (-2.0, 2.0, 1.0), (0.0, 0.0, 1.5), (2.0, -1.0, 1.5), (2.0, 1.0, 2.0), (3.0, 0.0, 2.0)]
...
print(
    f"[OPTIMIZATION] Initial guess source: X: {guess_params[0]} km, Y: {guess_params[1]} km, Z: {guess_params[2]} km (Amp: {guess_params[3]})"
)
...
print(f"\n=====================================================================")
print(
    f"[RESULTS] True Parameters:    X={true_params[0]:.2f}, Y={true_params[1]:.2f}, Z={true_params[2]:.2f}, Amp={true_params[3]:.2f}"
)
print(
    f"[RESULTS] Recovered Parameters: X={optimal_params[0]:.2f}, Y={optimal_params[1]:.2f}, Z={optimal_params[2]:.2f}, Amp={optimal_params[3]:.2f}"
)
print(f"=====================================================================")
...
parameter_history = np.stack([np.asarray(guess_params), *[np.asarray(value) for value in history["params"]]])
artifact = xr.Dataset(
    data_vars={
        "plume_snapshot": (("snapshot", "x", "y", "z"), np.asarray(plume_history[jnp.asarray(snapshot_indices)])),
        "physical_height": (("x", "y", "z"), np.asarray(grid.Z_m) / 1000.0),
        "terrain_height": (("x", "y"), np.asarray(terrain_profile(grid.x_m[:, None], grid.y_m[None, :])) / 1000.0),
        "target_sensor_concentration": (("time", "sensor"), np.asarray(target_sensor_history)),
        "recovered_sensor_concentration": (("time", "sensor"), np.asarray(recovered_sensor_history)),
        "parameter_history": (("optimization_state", "parameter"), parameter_history),
        "optimization_loss": ("optimization_step", np.asarray(history["loss"])),
        "sensor_location": (("sensor", "spatial_component"), np.asarray(sensor_locs)),
        "sensor_index": (("sensor", "index_component"), np.asarray(sensor_indices)),
    ...
    coords={
        ...
        "snapshot_time": ("snapshot", (np.asarray(snapshot_indices) + 1) * dt),
...
if not args.plot_inline:
    print(
        ...
        f"{sys.executable} experiments/tracer_inversion_3d_complex/render.py "
        f"{artifact_path}"
    )
    raise SystemExit(0)
...
for col_idx, step_idx in enumerate(snapshot_indices):
    ...
    c_top = ax_top.contourf(
        x_1d, y_1d, plume_at_t[:, :, true_z_idx].T, levels=levels, cmap="Blues", extend="max", zorder=1
    )
    ...
    topo_contours = ax_top.contour(
        x_1d,
        y_1d,
        terrain_2d.T,
        levels=[0.2, 0.5, 0.8, 1.1, 1.4, 1.7],
        colors="black",
        alpha=0.20,
        linewidths=0.8,
        zorder=1.6,
    ...
    ax_top.scatter(
        ...
        s=100,
        linewidths=2.0,
        label="True source" if col_idx == 0 else "",
        zorder=6,
    )
    ...
    ax_top.scatter(
        sensor_x_np, sensor_y_np, color="red", marker="^", s=80, label="Sensor towers", edgecolor="black", zorder=3
    )
    ...
    # Contour of plume concentration
    ax_bottom.contourf(
        X_grid_padded.T, Z_slice_padded.T, plume_slice_padded.T, levels=levels, cmap="Blues", extend="max"
    )
...
plt.savefig(figure_dir / f"tracer_3d_complex_plume_{CORE_TYPE.lower()}.png", dpi=150)
plt.close()
...
ax3.plot(
    hx, hy, marker="o", color="purple", linestyle="-", linewidth=2, markersize=4, label="Optimizer trajectory", zorder=4
)
...
ax4.plot(
    hx, hz, marker="o", color="purple", linestyle="-", linewidth=2, markersize=4, label="Optimizer trajectory", zorder=4
)
...
plt.tight_layout()
plt.savefig(figure_dir / f"tracer_3d_complex_optimization_{CORE_TYPE.lower()}.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(figure_dir / f"tracer_3d_complex_sensors_{CORE_TYPE.lower()}.png", dpi=150)
plt.close()

print(f"[DONE] Visualizations saved to {figure_dir}/")
