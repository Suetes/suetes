...
def _evaluate(transform, terrains, steps):
    ...
    for terrain in terrains:
        try:
            ...
            return {"mean_tke": np.inf, "std_tke": np.nan, "minimum_layer_m": np.nan, "samples": len(values), "error": str(error)}
...
def _worker(args, dataset, terrains, steps):
    if args.worker_kind == "direct":
        ...
        result = {"kind": "direct", "amplitude": args.worker_amplitude, **_evaluate(transform, terrains, steps)}
    elif args.worker_kind == "galchen":
        result = {"kind": "galchen", "amplitude": np.nan, **_evaluate(GalChenSigma(), terrains, steps)}
    else:
        with open(args.sleve_config) as stream:
            config = json.load(stream)
        transform = SleveSimple(scale_s=float(config["scale_s"]), scale_l=15000.0, n=float(config["n"]))
        result = {"kind": "sleve", "amplitude": np.nan, **_evaluate(transform, terrains, steps)}
...
def _plot(rows, output_dir, minimum_layer):
    direct = [row for row in rows if row["kind"] == "direct" and np.isfinite(row["mean_tke"])]
    ...
    axes[0].axhline(galchen["mean_tke"], color="0.45", linestyle="--", label="Gal--Chen")
    axes[0].axhline(sleve["mean_tke"], color="tab:orange", linestyle="--", label="Tuned SLEVE")
    axes[0].set(xlabel=r"Aggressiveness $a$", ylabel=r"Spurious TKE (m$^2$ s$^{-2}$)")
    axes[0].legend(frameon=False)

    axes[1].plot(amplitudes, layers, "o-", color="tab:green")
    axes[1].axhline(minimum_layer, color="tab:red", linestyle="--", label="Acceptance threshold")
    axes[1].axhline(sleve["minimum_layer_m"], color="tab:orange", linestyle=":", label="Tuned SLEVE")
    axes[1].set(xlabel=r"Aggressiveness $a$", ylabel=r"Minimum layer thickness (m)")
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("output/neuve_aggressiveness_scan"))
    parser.add_argument("--amplitudes", default="0,0.25,0.5,0.75,1,1.5,2,3,4,5", help="Comma-separated values in log(rho)=a(1-2 eta)")
    parser.add_argument("--basis-count", type=int, default=16)
    parser.add_argument("--minimum-layer-m", type=float, default=150.0)
    parser.add_argument("--worker-kind", choices=("direct", "galchen", "sleve"), help=argparse.SUPPRESS)
    ...
    def completed(kind, amplitude=None):
        return next((row for row in rows if row["kind"] == kind and (amplitude is None or np.isclose(row["amplitude"], amplitude))), None)
    ...
    with tempfile.TemporaryDirectory(prefix="suetes-neuve-scan-") as temporary:
        ...
        for index, amplitude in enumerate(_parse_values(args.amplitudes)):
            ...
            rows.append(_run_worker(args, "direct", temporary / f"direct_{index}.json", amplitude))
            save_progress()

    for row in rows:
        row["feasible"] = bool(np.isfinite(row["minimum_layer_m"]) and row["minimum_layer_m"] >= args.minimum_layer_m)
    ...
    for row in rows:
        ...
        print(f"{label:<12} TKE={row['mean_tke']:.6e} min_dz={row['minimum_layer_m']:.2f} m feasible={row['feasible']}")
