...
from experiments._shared.neuve_coordinate import load_dataset, run_target_case, terrains_from_dataset
from experiments._shared.neuve_learned_coordinate import LearnedDensityCoordinate
...
def _transform(amplitude, basis_count):
    return LearnedDensityCoordinate({"global": jnp.linspace(amplitude, -amplitude, basis_count)})
...
def _worker(args):
    ...
    def objective(amplitude):
        metric, _, grid, _ = run_target_case("pgf_rest", _transform(amplitude, args.basis_count), terrain, args.worker_steps)
    ...
    for epsilon in _values(args.epsilons):
        ...
        row[f"relative_error_{suffix}"] = abs(finite_difference - row["ad_gradient"]) / max(
            abs(finite_difference), abs(row["ad_gradient"]), 1.0e-14
        )
...
def _plot(rows, output_dir, reference_amplitude, full_steps, epsilon):
    suffix = f"{epsilon:.0e}"
    horizon = sorted((row for row in rows if np.isclose(row["amplitude"], reference_amplitude)), key=lambda row: row["steps"])
    amplitude = sorted((row for row in rows if row["steps"] == full_steps), key=lambda row: row["amplitude"])
    figure, axes = plt.subplots(1, 2, figsize=(9.0, 3.7))
    axes[0].plot([row["steps"] for row in horizon], [row["ad_gradient"] for row in horizon], "o-", label="Reverse-mode AD")
    axes[0].plot([row["steps"] for row in horizon], [row[f"fd_{suffix}"] for row in horizon], "s--", label="Centered finite difference")
    axes[0].axhline(0.0, color="0.4", linewidth=0.8)
    axes[0].set(xlabel="Integration steps", ylabel=r"$\partial\mathcal{J}/\partial a$")
    axes[0].legend(frameon=False)

    axes[1].plot([row["amplitude"] for row in amplitude], [row["ad_gradient"] for row in amplitude], "o-", label="Reverse-mode AD")
    axes[1].plot(
        [row["amplitude"] for row in amplitude], [row[f"fd_{suffix}"] for row in amplitude], "s--", label="Centered finite difference"
    )
    axes[1].axhline(0.0, color="0.4", linewidth=0.8)
    axes[1].set(xlabel=r"Aggressiveness $a$", ylabel=r"$\partial\mathcal{J}/\partial a$")
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("output/neuve_scalar_gradient_audit"))
    parser.add_argument("--basis-count", type=int, default=16)
    parser.add_argument("--terrain-index", type=int, default=0)
    parser.add_argument("--amplitudes", default="0,0.5,1,2", help="Full-horizon amplitudes")
    parser.add_argument("--horizons", default="25,50,100,250,500", help="Integration steps audited at the reference amplitude")
    ...
    def existing(amplitude, steps):
        return any(row["steps"] == steps and np.isclose(row["amplitude"], amplitude) for row in rows)
    ...
    with tempfile.TemporaryDirectory(prefix="suetes-gradient-audit-") as temporary:
        ...
        for index, (amplitude, steps) in enumerate(sorted(cases)):
            ...
            rows.append(_run_worker(args, amplitude, steps, temporary / f"case_{index}.json"))
            with open(progress_path, "w") as stream:
                json.dump(rows, stream, indent=2)

    rows.sort(key=lambda row: (row["steps"], row["amplitude"]))
    fieldnames = list(dict.fromkeys(key for row in rows for key in row))
    with open(args.output_dir / "scalar_gradient_audit.csv", "w", newline="") as stream:
    ...
    _plot(rows, args.output_dir, args.reference_amplitude, full_steps, epsilon)
