...
def main():
    ...
    if args.output_dir is None:
        layout = ArtifactLayout(
            kind="experiments", case="era5_4dvar",
    ...
    bc_states_native = bridge.build_or_load_timeseries(
        ...
        cache_path=state_cache_path, coarsen_window=None
    )
    ...
    z_0_field = bridge.process_static(raw_t0)['land_fraction'] * 0.1 + (1.0 - bridge.process_static(raw_t0)['land_fraction']) * 1e-4
    physics_suite.add_tendency_scheme(McFarlaneSurfaceDrag(grid, operators, constants, z_0=z_0_field, epsilon=jnp.zeros_like(z_0_field), theta_surf=initial_state_true['theta_surf']))
    physics_suite.add_tendency_scheme(McFarlaneVerticalDiffusion(grid, operators, constants, epsilon=jnp.zeros_like(z_0_field)[..., None]))
    physics_suite.add_tendency_scheme(NewtonianRelaxation(tau_relax_hours=6.0))

    stepper, _ = build_dynamical_core(
        core_type=CORE_TYPE, grid=grid, operators=operators, constants=constants, 
        initial_state=initial_state_true, physics_suite=physics_suite, 
        interior_mask=sponge.get_interior_mask(), dt=dt, ns=3, damp_height=12000.0, max_damp=3.0
    )
    ...
    def run_trajectory(start_state):
        ...
        def advance_one_obs_window(state, start_step_idx):
            def inner_step(s, i):
                ...
                def bc_fn(s_next, _): return sponge.blend(s_next, bc_state_t)
    ...
    def objective_fn(chi_dict):
        ...
        def bc_fn_t0(s, _): return sponge.blend(s, bc_state_t0)
    ...
    optimal_chi, history = solver.fit(
        chi_init, 
        total_steps=total_steps, 
        metric_name="Cost Function", 
        maximize=False
    )
    ...
    dataset = xr.Dataset(
        data_vars={
            "surface_field": (
                ...
                np.stack([
                    np.stack([
                        np.asarray(blurred_background[var][:, :, 0])
                        for var in variables
                    ]),
                    np.stack([
                        np.asarray(recovered_state[var][:, :, 0])
                        for var in variables
                    ]),
                    np.stack([
                        np.asarray(initial_state_true[var][:, :, 0])
                        for var in variables
                    ]),
                ]),
            ),
    ...
    artifact_path = save_plot_dataset(
        dataset, data_dir / "artifact.nc", experiment="era5_4dvar"
    )
    ...
    for i, var in enumerate(['th_v', 'u', 'pi']):
        ...
        im = axs[i, 2].imshow(initial_state_true[var][:, :, z_idx].T, origin='lower', vmin=vmin, vmax=vmax, cmap='viridis')
