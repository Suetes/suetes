...
def main():
    ...
    try:
        source = artifact_from_bundle(
            args.artifact, "gravity_wave_optimal_topography_*.nc"
        )
    ...
    for index, coefficients in enumerate(history):
        ...
        axis.plot(x_km, profiles[-1], color="blue",
                  alpha=0.5 * (index + 1) / len(history))
    best = int(data.attrs["best_optimization_index"])
    axis.plot(
        x_km, profiles[best], color="red", linewidth=2,
        label=f"Best (E={data.optimization_energy.values[best]:.1f} J)",
    )
    ...
    for index, (axis, label) in enumerate(zip(axes.flat, data.configuration.values)):
        ...
        axis.contourf(
            np.broadcast_to(x_km[:, None], z_km.shape), z_km, w,
            levels=np.linspace(-1.5, 1.5, 41), cmap="RdBu_r", extend="both",
        )
        axis.fill_between(
            x_km, 0, data.terrain_height.isel(configuration=index).values / 1000.0,
            color="black",
        )
        rect = patches.Rectangle(
            (x_km[180], z_km[180, 8]),
            x_km[200] - x_km[180], z_km[180, 20] - z_km[180, 8],
            linewidth=2, edgecolor="k", facecolor="none", linestyle="--",
        )
        axis.add_patch(rect)
        axis.set_title(
            f"{label}\nTarget energy (J): "
            f"{data.validation_energy.values[index]:.2f}", fontweight="bold"
        )
    ...
    fig.tight_layout()
    fig.savefig(
        output / f"inverse_topography_validation_{core}.png",
