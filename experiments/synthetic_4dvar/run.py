...
if args.output_dir is None:
    layout = ArtifactLayout(
        kind="experiments", case="synthetic_4dvar",
...
def h_func(x, y): return jnp.zeros_like(x)  # Flat terrain for clean test
...
true_th_v = bg_ref['th_v'] - 5.0 * jnp.exp(-((X_3d - 0.0)**2 / (2 * 10000.0**2)) - ((Z_3d - 2000.0)**2 / (2 * 2000.0**2)))
...
stepper, _ = build_dynamical_core(
    ...
    initial_state=true_initial_state, dt=dt, ns=4
)

def dummy_bc(state_in, forcing=None): return state_in
...
def apply_b_half(chi, sigma_x=5.0, sigma_z=2.5):
    ...
    def smooth_x(field_1d): return smooth_1d(field_1d, kernel_x)
    def smooth_z(field_1d): return smooth_1d(field_1d, kernel_z)
...
artifact = xr.Dataset(
    data_vars={
        "theta_anomaly": (
            ("state", "x", "z"),
            np.stack([anom_blurred, anom_recovered, anom_true]),
        ),
    ...
    attrs={
        ...
        "assimilation_window_s": assimilation_window,
    },
)
artifact_path = save_plot_dataset(
    artifact, data_dir / "artifact.nc", experiment="synthetic_4dvar"
)
print(f"[OUTPUT] Saved {artifact_path}")
if args.no_render:
    raise SystemExit(0)

im0 = axs[0].contourf(X_3d[:, 1, :]/1000, Z_3d[:, 1, :], anom_blurred, cmap='RdBu_r', levels=np.linspace(vmin, vmax, 21))
