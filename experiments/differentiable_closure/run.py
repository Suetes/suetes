...
def build_differentiable_step(stepper, sponge):
    def step_fn(curr_state, step_args):
        ...
        dynamic_theta_surf = local_land_frac * jnp.asarray(bc_state_t["theta_skt"], dtype=jnp.float64) + (
            1.0 - local_land_frac
        ) * jnp.asarray(bc_state_t["th_v"][:, :, 0], dtype=jnp.float64)
...
# ---------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Train the differentiable closure")
    ...
    full_grid = RegionalGrid3D(
        nx_full, ny_full, nz, dx, dy, dz, 47.5, -97.0, h_func=h_func_full, transform=SleveSimple(scale_s=10000.0, n=1.0)
    )
    ...
    for p_idx in range(num_training_patches):
        ...
        patch_grid = RegionalGrid3D(
            ...
            center_lon,
            h_func=patch_h_func,
            transform=SleveSimple(scale_s=10000.0, n=1.0),
        )
        ...
        init_state["theta_surf"] = (
            init_state["land_fraction"] * patch_states[0]["theta_skt"]
        ...
        physics_ml.add_tendency_scheme(
            McFarlaneSurfaceDrag(
                ...
                theta_surf=init_state["theta_surf"],
            )
        )
        physics_ml.add_tendency_scheme(
            McFarlaneVerticalDiffusion(patch_grid, patch_ops, constants, epsilon=epsilon_field[..., None])
        )
        ...
        core_ml = Euler3D(
            ...
            constants,
            dt=dt,
            initial_era5_state=init_state,
            physics_suite=physics_ml,
            interior_mask=patch_sponge.get_interior_mask(),
        )
        ...
        scm_models[f"patch_{p_idx}"] = {
            ...
            "time_manager": patch_time_manager,
            "sponge": patch_sponge,
            "step_fn": step_fn,
    ...
    optimizer = optax.chain(optax.clip_by_global_norm(10.0), optax.adam(learning_rate=1e-3))
    ...
    scan_inputs = (
        init_state_stack,
        land_stack,
        jnp.stack(targ_th_list),
        jnp.stack(targ_u_list),
        jnp.stack(targ_v_list),
        bc_stack,
    )
    ...
    checkpoint = output_dir / "universal_ml_closure.pkl"
    with checkpoint.open("wb") as f:
        pickle.dump({"params": ml_params, "norm_stats": norm_stats}, f)
    print(f"\n[SUCCESS] Universal ML parameters saved to {checkpoint}")
