...
if args.output_dir is None:
    layout = ArtifactLayout(
        kind="experiments", case="tracer_inversion_3d",
...
artifact = xr.Dataset(
    data_vars={
        "tracer_snapshot": (
            ("time", "x", "z"), np.asarray(snapshots)
        ),
        "terrain_height": ("x", np.asarray(terrain_plot) * 1000.0),
        "physical_height": (
            ("x", "z"), np.asarray(grid.Z_m[:, 1, :])
        ),
        "optimization_parameters": (
            ("optimization_step", "parameter"),
            np.asarray(history["params"]),
        ),
        "target_sensor_profile": (
            "z", np.asarray(target_sensor_profile)
        ),
        "recovered_sensor_profile": (
            "z", np.asarray(recovered_sensor_profile)
        ),
    },
    coords={
...
artifact_path = save_plot_dataset(
    artifact, data_dir / "artifact.nc", experiment="tracer_inversion_3d"
)
