"""
Unified CORDEX NAM-22 Downscaling Experiment.
...
outbreak. Dynamically switches between SISL and Split-Explicit cores.
"""
...
def main():
    ...
    RUN_NAME = f"{ACTIVE_DOMAIN}_NAM22_july2025_{CORE_TYPE.lower()}"
    ...
    # Define core-specific timeline and stability configuration
    if CORE_TYPE.lower() == "sisl":
        dt = 120.0
        core_kwargs = {
            "dt": dt, "nu_div_factor": 0.1, "nu_h_factor": 0.1, 
            "damp_height": 9000.0, "max_damp": 3.0
        }
    elif CORE_TYPE.lower() == "split-explicit":
        dt = 40.0
        core_kwargs = {
            "dt": dt, "ns": 3, "nu_div_factor": 0.03, "nu_h_factor": 0.03, 
            "damp_height": 12000.0, "max_damp": 3.0
        }
    else:
        raise ValueError("Invalid CORE_TYPE.")
    ...
    # ---------------------------------------------------------
    # 2. SETUP PROCESSORS & STATIC FIELDS
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 3. AUTOMATED BOUNDARY CACHING & LOAD
    # ---------------------------------------------------------
    ...
    suetes_bc_states = bridge.build_or_load_timeseries(
        era5_proc=era5_proc, num_states=num_era5_states,
        cache_path=bc_cache_path_coarse, coarsen_window=coarsen_window
    )

    print(f"[BOUNDARY] Preparing native reference BC states...")
    suetes_bc_states_native = bridge.build_or_load_timeseries(
        era5_proc=era5_proc, num_states=num_era5_states,
        cache_path=bc_cache_path_native, coarsen_window=None
    )
    ...
    # ---------------------------------------------------------
    # 6. DYNAMICAL CORE FACTORY
    # ---------------------------------------------------------
    print(f"[DYNAMICS] Initializing {CORE_TYPE.upper()} dynamical core...")
    stepper, dt = build_dynamical_core(
        core_type=CORE_TYPE, grid=grid, operators=operators, 
    ...
    # Calculate chunk size to print progress precisely every 1 simulated hour
    chunk_steps = max(1, int(3600.0 / dt))
    ...
    for z in [0, 5, 15, 30]:
        visualizer.plot_dashboard(
            grid, final_state, z_idx=z, sponge_depth=sponge_depth, time_hours=sim_hours,
            save_path=os.path.join(output_dir, f"{RUN_NAME}_dash_z{z}_{sim_hours}h.png"),
        )

    for z in [500.0, 3000.0, 5000.0, 10000.0]:
        visualizer.plot_dashboard(
            grid, final_state, z_idx=z, sponge_depth=sponge_depth, time_hours=sim_hours,
            save_path=os.path.join(output_dir, f"{RUN_NAME}_dash_z{int(z)}m_{sim_hours}h.png"),
        )

    visualizer.plot_energy_spectrum(
        grid, final_state, 'w', z_idx=5, sponge_depth=sponge_depth,
        save_path=os.path.join(output_dir, f"{RUN_NAME}_energy_{sim_hours}h.png"),
    )

    visualizer.plot_comparison(
        grid, final_state, final_era5_state, 'th_v', z_idx=0, sponge_depth=sponge_depth,
        save_path=os.path.join(output_dir, f"{RUN_NAME}_compare_th_v_surf_{sim_hours}h.png"),
    )

    visualizer.plot_level_strip(
        grid, final_state, 'th_v', z_indices=[0, 5, 15, 30], sponge_depth=sponge_depth,
        save_path=os.path.join(output_dir, f"{RUN_NAME}_levels_th_v_{sim_hours}h.png"),
    )

    visualizer.plot_hovmoller(
        grid, hov_times, np.array(hov_data), variable='Surface th_v Anomaly [K]',
        save_path=os.path.join(output_dir, f"{RUN_NAME}_hovmoller_th_v_{sim_hours}h.png"),
    )

    visualizer.plot_slice_locator_dashboard(
        grid, final_state, map_var='th_v', slice_var='w', map_z=5, sponge_depth=sponge_depth,
        save_path=os.path.join(output_dir, f"{RUN_NAME}_slices_w_{sim_hours}h.png"),
    )
    ...
    print(f"{'Location':<14}{'i':>5}{'j':>5}{'Z_m[0] [m]':>13}{'th_v [K]':>11}{'pi':>9}{'T [K]':>9}{'T [C]':>9}")
    ...
    for name, p_lat, p_lon in POINTS:
        ...
        T = th_v * pi
        print(f"{name:<14}{i_p:>5d}{j_p:>5d}{z0:>13.1f}{th_v:>11.2f}{pi:>9.4f}{T:>9.2f}{T-273.15:>9.2f}")
    print("-" * 60)
    ...
    for name, p_lat, p_lon in POINTS:
        ...
        suetes_T_C = [float(s['th_v'][i_p, j_p, 0] * s['pi'][i_p, j_p, 0]) - 273.15 for s in snapshots]
        era5_T_C = [float(s['th_v'][i_p, j_p, 0] * s['pi'][i_p, j_p, 0]) - 273.15 for s in suetes_bc_states_native]
        ...
        visualizer.plot_point_timeseries(
            times_hours_pt, suetes_T_C, era5_T_C, location_name=location_str, units='deg C',
            save_path=os.path.join(output_dir, f"{RUN_NAME}_T_{safe_name}_{sim_hours}h.png"),
        )
