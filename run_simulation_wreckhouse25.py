...
severe downslope wind event. Dynamically switches between SISL and 
Split-Explicit cores.
"""
...
def main():
    ...
    RUN_NAME = f"{ACTIVE_DOMAIN}_2km_feb2025_{CORE_TYPE.lower()}"
    ...
    # Define core-specific timeline and stability configuration for 2km res
    if CORE_TYPE.lower() == "sisl":
        dt = 5.0
        core_kwargs = {
            "dt": dt, "nu_div_factor": 0.2, "nu_h_factor": 0.2, 
            "damp_height": 9000.0, "max_damp": 3.0
        }
    elif CORE_TYPE.lower() == "split-explicit":
        dt = 5.0
        core_kwargs = {
            "dt": dt, "ns": 4, "nu_div_factor": 0.05, "nu_h_factor": 0.05, 
            "damp_height": 9000.0, "max_damp": 3.0
        }
    else:
    ...
    for h in target_hours:
        if h < len(snapshots):
            ...
            # Vertical Slice Locator Dashboard
            visualizer.plot_slice_locator_dashboard(
    ...
    # ---------------------------------------------------------
    # 6. TIME SERIES EXTRACTION FOR LOCAL POINTS
    # ---------------------------------------------------------
    POINTS = [
    ...
    for name, p_lat, p_lon in POINTS:
        ...
        for s in snapshots:
            ...
            suetes_wind.append(float(jnp.sqrt(u_p**2 + v_p**2)) * 3.6)
